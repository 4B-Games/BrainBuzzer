...
export default function SettingsView({ onDataChange, currentUser }) {
  ...
  function addCompany() {
    ...
    if (!name) return
    persist([...companies, { id: uid(), name, color: newCompanyColor, projects: [] }])
    setNewCompanyName('')
  ...
  function addProject(companyId) {
    ...
    persist(companies.map(c =>
      c.id === companyId
        ? { ...c, projects: [...c.projects, { id: uid(), name, emoji }] }
        : c
    ))
  ...
  return (
    <div className="view">
      <div className="view-header">
        <h1>Einstellungen</h1>
        {!isAdmin && (
          <span className="settings-readonly-hint">Nur Admins können Unternehmen und Projekte löschen.</span>
        )}
      </div>
                  ...
                  {isAdmin && (
                    confirmDelete === company.id ? (
                      <>
                        ...
                        <button className="btn-danger-sm" onClick={() => deleteCompany(company.id)}>Ja</button>
                        <button className="btn-secondary-sm" onClick={() => setConfirmDelete(null)}>Nein</button>
                      </>
                    ) : (
                      <button className="btn-icon-danger" onClick={() => setConfirmDelete(company.id)} title="Unternehmen löschen">
                        <Trash2 size={15} />
                      </button>
                    )
                  )}
                  ...
                  <ul className="settings-project-list">
                    {company.projects.map(p => (
                      <li key={p.id} className="settings-project-item">
                        ...
                        <span>{p.name}</span>
                        {isAdmin && (
                          <button className="btn-icon-danger" onClick={() => deleteProject(company.id, p.id)} title="Projekt löschen">
                            <Trash2 size={13} />
                          </button>
                        )}
                      </li>
